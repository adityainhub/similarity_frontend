...
const SolutionDetailsPageUserName = () => {
  const { username, contestId, questionId } = useParams();
  ...
  useEffect(() => {
    if (username && contestId && questionId) {
      fetch(`${BASE_URL}/api/contests/username-search/${username}/${contestId}/${questionId}`)
        .then(response => response.json())
        .then(data => {
          setSolutionData(data);
          
          // Convert the data to match the expected format
          const mainUserData: UserDetail = {
            id: 1,
            rank: data.mainUser.rank,
            username: data.mainUser.username,
            language: data.mainUser.language,
            solved: true,
            timeSeconds: 0,
            matchCount: data.totalMatches,
            score: 0,
            submissions: 1,
            profileInitials: data.mainUser.username.charAt(0).toUpperCase()
          };
          setActiveUser(mainUserData);

          // Convert matched users to similarity matches
          const similarityMatches: SimilarityMatch[] = data.matchedUsers.map(user => ({
            submissionId1: data.mainUser.submissionId,
            submissionId2: user.submissionId,
            username1: data.mainUser.username,
      ...
      >
        ...
        <div className="flex justify-center mt-4">
          <CodeButton
            ...
            isLeftPanel={true}
            submissionId={solutionData.mainUser.submissionId}
            contestId={contestId}
        ...
        <div className="bg-[#1a1a1a] border border-[#333] rounded-lg overflow-hidden">
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            {similarityData.map((match, index) => {
              const isUser1Searched = match.username1 === username;
      ...
      <div className="container mx-auto p-4">
        ...
        >
          <Button
            ...
            onClick={() => navigate(`/username-search/${username}/${contestId}`)}
          >
...
export default SolutionDetailsPageUserName; 